...
OLD_VERSION=$(sed 's/.*\([0-9]\+\.[0-9]\+\.[0-9]\+\)$/\1/' <<<"$OLD_VERSION")
if [[ $HOTFIX == "true" ]]; then
  BASE=$(sed 's/^.*\([0-9]\+\.[0-9]\+\.[0-9]\+\)-.*/\1/' <<<"$OLD_VERSION")
  [[ $OLD_VERSION =~ "-" ]] &&
    HOTFIX=$(sed 's:^.*-::g' <<<"$OLD_VERSION") || HOTFIX=0

  HOTFIX=$((HOTFIX + 1))
  VERSION="v$BASE-$HOTFIX"
else
  MAJOR=$(sed 's/^\([0-9]\+\)\.[0-9]\+\.[0-9]\+.*$/\1/' <<<"$OLD_VERSION")
  MINOR=$(sed 's/^[0-9]\+\.\([0-9]\+\)\.[0-9]\+.*$/\1/' <<<"$OLD_VERSION")
